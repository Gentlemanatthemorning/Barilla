...
(function () {
  var defaultSettings = {
    ...
    labelError = function (e, input, check, settings) {
      ...
      var textMissing =
        input.getAttribute("data-missing") || settings.messages.missing;
      if (validity.valueMissing && textMissing) {
        input.setCustomValidity(textMissing);
      }

      var textMismatch =
        input.getAttribute("data-mismatch") || settings.messages.mismatch;
      if (
  ...
  $.fn.validity = function (settings) {
    ...
    var $forms = $(this),
      ...
      selector =
        settings.selector + ":not(" + settings.ignore + "):not(:button)",
